...
function db(): PDO
{
    global $cfg;
    static $db;
    return $db ??= new PDO("mysql:host={$cfg['db_host']};dbname={$cfg['db_name']};charset=utf8mb4", $cfg['db_user'], $cfg['db_pass'], [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION, PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC]);
}
...
function allow(array $roles): array
{
    $u = user();
...
function fields(array $r): array
{
    $map = ['sacrament' => 'sacrament_type', 'firstName' => 'first_name', 'middleName' => 'middle_name', 'lastName' => 'last_name', 'gender' => 'gender', 'dob' => 'date_of_birth', 'eventDate' => 'event_date', 'fatherName' => 'father_name', 'motherName' => 'mother_maiden_name', 'spouse' => 'spouse_name', 'book' => 'book_number', 'page' => 'page_number', 'line' => 'line_number', 'minister' => 'minister_name', 'sponsors' => 'sponsors', 'marginNotes' => 'margin_notes'];
    $out = [];
    foreach ($map as $a => $b)
        $out[$b] = clean($r[$a] ?? $r[$b] ?? '');
    return $out;
}
function validate(array $r, bool $database = true): array
{
    $f = fields($r);
    $e = [];
    $types = ['Baptism', 'Communion', 'Confirmation', 'Marriage', 'Death'];
    if (!in_array($f['sacrament_type'], $types, true))
        $e[] = 'Missing Sacrament Type';
    if (!$f['first_name'] || !$f['last_name'])
        $e[] = 'First and last name required';
    if (!in_array($f['gender'], ['Male', 'Female'], true))
        $e[] = 'Gender must be Male or Female';
    foreach (['event_date', 'book_number', 'page_number', 'line_number'] as $x)
        if (!$f[$x])
            $e[] = "$x is required";
    foreach (['date_of_birth', 'event_date'] as $x)
        if ($f[$x] && !preg_match('/^\\d{4}-\\d{2}-\\d{2}$/', $f[$x]))
            $e[] = 'Invalid Date';
    if ($f['date_of_birth'] && $f['event_date'] && $f['event_date'] < $f['date_of_birth'])
        $e[] = 'Invalid Date';
    if (in_array($f['sacrament_type'], ['Baptism', 'Communion', 'Confirmation'], true)) {
        if (!$f['father_name'])
            $e[] = 'Missing Father Name';
        if (!$f['mother_maiden_name'])
            $e[] = 'Missing Mother Name';
    }
    if ($database && !$e) {
        $s = db()->prepare('SELECT id FROM sacramental_records WHERE sacrament_type=? AND book_number=? AND page_number=? AND line_number=?');
        $s->execute([$f['sacrament_type'], $f['book_number'], $f['page_number'], $f['line_number']]);
        if ($s->fetch())
            $e[] = 'Duplicate Entry';
    }
    return [$f, array_values(array_unique($e))];
}
function record(int $id): array|false
{
    $s = db()->prepare('SELECT r.id,r.sacrament_type sacrament,r.event_date eventDate,r.book_number book,r.page_number page,r.line_number line,r.minister_name minister,r.sponsors,r.margin_notes marginNotes,p.first_name firstName,p.middle_name middleName,p.last_name lastName,p.gender,p.date_of_birth dob,p.father_name fatherName,p.mother_maiden_name motherName,p.spouse_name spouse FROM sacramental_records r JOIN persons p ON p.id=r.person_id WHERE r.id=?');
    $s->execute([$id]);
    return $s->fetch();
}
function settings(): array
{
    $s = db()->query('SELECT * FROM parish_settings WHERE id=1')->fetch();
    $rows = db()->query('SELECT * FROM certificate_templates')->fetchAll();
    $templates = [];
    foreach ($rows as $row)
        $templates[$row['sacrament_type']] = $row;
    return ['parish' => $s, 'templates' => $templates];
}
function save_upload(string $field, string $prefix): ?string
{
    ...
    if ($f['size'] > 2 * 1024 * 1024)
        json(['error' => 'Image must be under 2MB'], 422);
    $mime = mime_content_type($f['tmp_name']);
    $ext = match ($mime) { 'image/png' => 'png', 'image/jpeg' => 'jpg', default => null};
    if (!$ext)
        json(['error' => 'Only PNG or JPEG images are allowed'], 422);
    $dir = __DIR__ . '/../uploads';
    if (!is_dir($dir) && !mkdir($dir, 0755, true))
        json(['error' => 'Could not store image'], 500);
...
if ($path === '/auth/login' && $method === 'POST') {
    $d = input();
    $key = clean($d['username'] ?? '');
    $s = db()->prepare('SELECT id,full_name,username,email,avatar_path,role,password_hash FROM staff_users WHERE (username=? OR email=?) AND is_active=1 LIMIT 1');
    $s->execute([$key, $key]);
    $u = $s->fetch();
    if (!$u || !password_verify((string) ($d['password'] ?? ''), $u['password_hash']))
        json(['error' => 'Invalid credentials'], 401);
    $_SESSION['user'] = ['id' => (int) $u['id'], 'full_name' => $u['full_name'], 'username' => $u['username'], 'email' => $u['email'], 'role' => $u['role'], 'avatar_path' => $u['avatar_path'] ?? null];
    json(['data' => $_SESSION['user']]);
}
...
if ($path === '/me' && $method === 'PUT') {
    ...
    try {
        db()->prepare('UPDATE staff_users SET full_name=?,email=? WHERE id=?')->execute([$fullName, $email ?: null, $u['id']]);
    } catch (PDOException $e) {
        json(['error' => 'Email address is already in use'], 422);
    }
...
if ($path === '/me/password' && $method === 'PUT') {
    ...
    $s = db()->prepare('SELECT password_hash FROM staff_users WHERE id=?');
    $s->execute([$u['id']]);
    $row = $s->fetch();
    if (!$row || !password_verify($current, $row['password_hash']))
        json(['error' => 'Current password is incorrect'], 422);
    db()->prepare('UPDATE staff_users SET password_hash=? WHERE id=?')->execute([password_hash($new, PASSWORD_DEFAULT), $u['id']]);
    json(['data' => true]);
}
if ($path === '/me/avatar' && $method === 'POST') {
    $u = user();
    $p = save_upload('avatar', 'avatar_' . $u['id']);
    if (!$p)
        json(['error' => 'No valid image uploaded'], 422);
    db()->prepare('UPDATE staff_users SET avatar_path=? WHERE id=?')->execute([$p, $u['id']]);
    $_SESSION['user']['avatar_path'] = $p;
    json(['data' => $_SESSION['user']]);
}
...
if ($path === '/settings/seal' && $method === 'POST') {
    ...
    db()->prepare('UPDATE parish_settings SET seal_image_path=? WHERE id=1')->execute([$p]);
    json(['data' => settings()]);
}
if ($path === '/settings/priest-signature' && $method === 'POST') {
    ...
    db()->prepare('UPDATE parish_settings SET priest_signature_path=? WHERE id=1')->execute([$p]);
    json(['data' => settings()]);
}
...
if ($path === '/records' && $method === 'GET') {
    ...
    foreach (['sacrament' => 'sacrament_type', 'dob' => 'date_of_birth', 'book' => 'book_number', 'page' => 'page_number', 'line' => 'line_number'] as $k => $c)
        if (!empty($_GET[$k])) {
            $where .= " AND " . (str_starts_with($c, 'date_') ? 'p' : 'r') . ".$c=?";
            $args[] = clean($_GET[$k]);
        }
    $page = max(1, (int) ($_GET['page_num'] ?? 1));
    $per = min(100, max(1, (int) ($_GET['per_page'] ?? 25)));
    $count = db()->prepare('SELECT COUNT(*) FROM sacramental_records r JOIN persons p ON p.id=r.person_id' . $where);
    $count->execute($args);
    $s = db()->prepare('SELECT r.id,r.sacrament_type sacrament,r.event_date eventDate,r.book_number book,r.page_number page,r.line_number line,r.minister_name minister,r.sponsors,r.margin_notes marginNotes,p.first_name firstName,p.middle_name middleName,p.last_name lastName,p.gender,p.date_of_birth dob,p.father_name fatherName,p.mother_maiden_name motherName,p.spouse_name spouse FROM sacramental_records r JOIN persons p ON p.id=r.person_id' . $where . ' ORDER BY r.event_date DESC LIMIT ' . $per . ' OFFSET ' . (($page - 1) * $per));
    $s->execute($args);
...
if ($path === '/records' && $method === 'POST') {
    ...
    try {
        ...
        $p->execute([$f['first_name'], $f['last_name'], $f['date_of_birth'] ?: null]);
        $pid = $p->fetchColumn();
        if (!$pid) {
        ...
        $d->rollBack();
        json(['error' => 'Could not save record'], 409);
    }
}
if (preg_match('#^/records/(\\d+)/issue$#', $path, $m) && $method === 'POST') {
    ...
    $x = input();
    $name = clean($x['requestor_name'] ?? '');
    $purpose = clean($x['purpose'] ?? '');
    if (!$name || !$purpose)
        json(['error' => 'Requestor name and purpose are required'], 422);
    $s = db()->prepare('INSERT INTO certificate_issuance_logs(sacramental_record_id,requestor_name,purpose,issued_by) VALUES(?,?,?,?)');
    $s->execute([$r['id'], $name, $purpose, $u['id']]);
...
if (preg_match('#^/records/(\d+)/certificate\.pdf$#', $path, $m) && $method === 'GET') {
    ...
    $issuance = $s->fetch();
    if (!$r || !$issuance)
        json(['error' => 'Issuance log required'], 404);
...
if ($path === '/imports' && $method === 'POST') {
    ...
    while (($v = fgetcsv($h)) !== false) {
        $raw = array_combine($headers, array_pad($v, count($headers), ''));
        [$f, $e] = validate($raw);
        $rows[] = ['row' => count($rows) + 2, 'data' => $raw, 'status' => $e ? 'Invalid' : 'Valid', 'errors' => $e];
    }
    fclose($h);
    $d = db();
    $s = $d->prepare('INSERT INTO import_batches(filename,uploaded_by,total_rows,valid_rows) VALUES(?,?,?,?)');
    $valid = count(array_filter($rows, fn($x) => $x['status'] === 'Valid'));
    $s->execute([basename($_FILES['file']['name']), $u['id'], count($rows), $valid]);
    $id = (int) $d->lastInsertId();
    $s = $d->prepare('INSERT INTO import_staged_rows(batch_id,row_number,payload,validation_errors) VALUES(?,?,?,?)');
    foreach ($rows as $r)
        $s->execute([$id, $r['row'], json_encode($r['data']), json_encode($r['errors'])]);
...
if (preg_match('#^/imports/(\\d+)/commit$#', $path, $m) && $method === 'POST') {
    ...
    $batch = $b->fetch();
    if (!$batch)
        json(['error' => 'Staged batch not found'], 404);
    $rows = $d->prepare('SELECT payload FROM import_staged_rows WHERE batch_id=?');
    $rows->execute([$batch['id']]);
    $d->beginTransaction();
    try {
        $n = 0;
        foreach ($rows as $row) {
            [$f, $e] = validate(json_decode($row['payload'], true));
            if ($e)
                continue;
            $p = $d->prepare('SELECT id FROM persons WHERE first_name=? AND last_name=? AND date_of_birth <=> ? LIMIT 1');
            $p->execute([$f['first_name'], $f['last_name'], $f['date_of_birth'] ?: null]);
            ...
            $s = $d->prepare("INSERT INTO sacramental_records(person_id,sacrament_type,event_date,book_number,page_number,line_number,minister_name,sponsors,margin_notes,source,created_by) VALUES(?,?,?,?,?,?,?,?,?,'bulk_import',?)");
            $s->execute([$pid, $f['sacrament_type'], $f['event_date'], $f['book_number'], $f['page_number'], $f['line_number'], $f['minister_name'] ?: null, $f['sponsors'] ?: null, $f['margin_notes'] ?: null, $u['id']]);
            $n++;
        }
        $d->prepare("UPDATE import_batches SET committed_rows=?,status='committed' WHERE id=?")->execute([$n, $batch['id']]);
        $d->commit();
        json(['data' => ['committed_rows' => $n]]);
    } catch (Throwable $e) {
        $d->rollBack();
        json(['error' => 'Import commit failed'], 409);
    }
}
...
if ($path === '/issuance-logs' && $method === 'GET') {
    ...
    $where = '';
    $args = [];
    if (!empty($_GET['sacrament'])) {
        $where = ' WHERE r.sacrament_type=?';
        $args[] = clean($_GET['sacrament']);
    }
    $count = db()->prepare('SELECT COUNT(*) FROM certificate_issuance_logs l JOIN sacramental_records r ON r.id=l.sacramental_record_id' . $where);
    $count->execute($args);
    $s = db()->prepare('SELECT l.id,l.requestor_name issuedTo,l.purpose,l.issued_at timestamp,u.full_name issuedBy,r.sacrament_type sacrament,CONCAT_WS(\' \',p.first_name,p.middle_name,p.last_name) personName FROM certificate_issuance_logs l JOIN sacramental_records r ON r.id=l.sacramental_record_id JOIN persons p ON p.id=r.person_id JOIN staff_users u ON u.id=l.issued_by' . $where . ' ORDER BY l.issued_at DESC LIMIT ' . $per . ' OFFSET ' . (($page - 1) * $per));
    $s->execute($args);
    json(['data' => $s->fetchAll(), 'meta' => ['total' => (int) $count->fetchColumn(), 'page' => $page, 'per_page' => $per]]);
}
if (!str_starts_with($_SERVER['REQUEST_URI'], '/api/v1')) {
    readfile(__DIR__ . '/parish-registry.html');
    exit;
}
json(['error' => 'Not found'], 404);
function certificate_html(array $r, string $requestor, string $purpose): string
{
    ...
    $cfg = settings();
    $parish = $cfg['parish'] ?? [];
    $tpl = $cfg['templates'][$r['sacrament']] ?? [];
    $name = trim($r['firstName'] . ' ' . ($r['middleName'] ?? '') . ' ' . $r['lastName']);
    $today = date('F j, Y');
    $token = fn($value) => "<strong style='color:#101010;'>" . $esc($value) . '</strong>';

    $tokens = [
        '{name}' => $token($name),
        '{dob}' => $token($date($r['dob'])),
        '{fatherName}' => $token($r['fatherName']),
        '{motherName}' => $token($r['motherName']),
        '{eventDate}' => $token($date($r['eventDate'])),
        '{spouse}' => $token($r['spouse']),
        '{sacrament}' => $token($r['sacrament']),
        '{book}' => $esc($r['book']),
        '{page}' => $esc($r['page']),
        '{line}' => $esc($r['line']),
    ];

    $body = strtr($esc($tpl['body_template'] ?? ''), $tokens);
...
SVG;
    ...
    $marginNotes = !empty($r['marginNotes'])
        ? "<table width='100%' style='margin:0 0 30px;'><tr><td style='background:#f3e0e3;border-left:4px solid #7a2734;padding:14px 18px;'>
             <span style='display:block;font-size:12px;font-weight:bold;color:#5e1f29;letter-spacing:.5px;'>CANONICAL MARGIN NOTES</span>
             <span style='color:#3a1015;font-size:14px;'>{$esc($r['marginNotes'])}</span>
           </td></tr></table>"
        : '';

    $title = $tpl['title_text'] ?? ('Certificate of ' . $r['sacrament']);
    $footer = $tpl['footer_note'] ?? 'Not valid without the parish dry seal.';
    ...
    <html><head><style>
      ...
      <p style="text-align:center;font-size:28px;font-weight:bold;color:#101010;margin:8px 0 4px;">{$esc($parish['parish_name'] ?? '')}</p>
      ...
      <p style="font-size:16px;line-height:2.1;text-align:justify;color:#161616;margin:0 0 36px;">{$body}</p>

      <table width="100%" style="border-top:1px solid #d8d2c2;border-bottom:1px solid #d8d2c2;margin-bottom:34px;"><tr>
        <td style="padding:16px 12px;font-size:13px;color:#101010;"><span style="display:block;font-size:11px;color:#4a4a4a;margin-bottom:2px;">Book No.</span><strong style="color:#101010;">{$esc($r['book'])}</strong></td>
        <td style="padding:16px 12px;font-size:13px;color:#101010;"><span style="display:block;font-size:11px;color:#4a4a4a;margin-bottom:2px;">Page No.</span><strong style="color:#101010;">{$esc($r['page'])}</strong></td>
        <td style="padding:16px 12px;font-size:13px;color:#101010;"><span style="display:block;font-size:11px;color:#4a4a4a;margin-bottom:2px;">Line Entry No.</span><strong style="color:#101010;">{$esc($r['line'])}</strong></td>
      </tr></table>

      {$marginNotes}

      <table width="100%" style="margin-top:50px;"><tr>
        <td width="50%" style="vertical-align:bottom;">{$signature}<div style="border-top:1px solid #101010;width:210px;margin-bottom:8px;"></div><strong style="color:#101010;font-size:14px;">{$esc($priest)}</strong><br/><span style="font-size:12px;color:#4a4a4a;">Officiating Priest / Parish Priest</span></td>
        <td width="50%" align="right" style="vertical-align:bottom;"><div style="border-top:1px solid #101010;width:170px;margin-bottom:8px;margin-left:auto;"></div><span style="font-size:12px;color:#4a4a4a;">Date Issued</span><br/><strong style="color:#101010;font-size:14px;">{$today}</strong></td>
      </tr></table>

      <p style="text-align:center;font-size:12px;color:#4a4a4a;border-top:1px dashed #c3cbd9;padding-top:18px;margin-top:50px;">Issued to <strong style="color:#101010;">{$issuedTo}</strong> for the purpose of <strong style="color:#101010;">{$esc($purpose)}</strong>. <span style="color:#4a4a4a;">{$esc($footer)}</span></p>
    </body></html>
    HTML;
}

function certificate_html_legacy(array $r, string $requestor, string $purpose): string
{
